...
/**
 *
 * Initialize the SAP HANA connector against the given data source
 *
 * @param {DataSource} dataSource The loopback-datasource-juggler dataSource
 * @callback {Function} [callback] The callback function
 * @header SAPHANA.initialize(dataSource, [callback])
 */
exports.initialize = function initializeDataSource(dataSource, callback) {
    ...
    dataSource.connector = new SAPHANA(saphana, dbSettings);
...
/**
 * SAP HANA connector constructor
 *
 * @param {SAPHANA} saphana SAPHANA node.js binding
 * @options {Object} settings An object for the data source settings.
 * Other parameters can be defined as query string of the url
 * @property {String} hostname The host name or ip address of the SAPHANA DB server
 * @property {Number} port The port number of the SAPHANA DB Server
 ...
 * @constructor
 */
function SAPHANA(saphana, settings) {
    this.constructor.super_.call(this, 'saphana', settings);
    this.saphana = new saphana.Client(settings);
...
require('util').inherits(SAPHANA, SqlConnector);

SAPHANA.prototype.debug = function () {
...
SAPHANA.prototype.connect = function (callback) {
    ...
    this.saphana.connect(function (err) {
        if (!err) {
            self.connection = self.saphana;
 ...
 */
SAPHANA.prototype.executeSQL = function (sql, params, callback) {
    var self = this;
    var time = Date.now();
    var log = self.log;
    ...
    if (params && params.length > 0) {
        self.connection.prepare(sql, function (err, statement) {
            if (err) {
        ...
        self.connection.exec(sql, function (err, data) {
 ...
 * @param {Object[]} data The result from the SQL
 */
SAPHANA.prototype.query = function (sql, params, callback) {
 ...
 * @private
 */
SAPHANA.prototype._categorizeProperties = function(model, data) {
...
/**
 * Create the data model in SAP HANA
 *
 * @param {String} model The model name
...
SAPHANA.prototype.create = function (model, data, callback) {
    ...
    var sql = "INSERT INTO " + tblName + " (" + fieldsAndData.fields + ")" + "\n";
...
/**
 * Save the model instance to SAP HANA
 * @param {String} model The model name
 * @param {Object} data The model instance data
 * @callback {Function} [callback] The callback function
 * @param {String|Error} err The error string or object
...
SAPHANA.prototype.update =
    SAPHANA.prototype.updateAll = function (model, where, data, callback) {
...
SAPHANA.prototype.buildInsert = function (model, data) {
...
SAPHANA.prototype.toDatabase = function (prop, val) {
    ...
    if (val.constructor.name === 'Object') {
        if (prop.saphana && prop.saphana.dataType === 'json') {
...
SAPHANA.prototype.schemaName = function (model) {
    // Check if there is a 'schema' property for saphana
    var dbMeta = this._models[model].settings && this._models[model].settings.saphana;
...
SAPHANA.prototype.dropTable = function (model, cb) {
    ...
    var dropTableFun = function (callback) {
        self.query('DROP TABLE ' + name + ' CASCADE', function (err, data) {
            callback(err, data);
        });
    };
...
SAPHANA.prototype.columnDataType = function (model, property) {
    ...
    switch (prop.type.name) {
        ...
        case 'Timestamp':
            return 'TIMESTAMP WITH TIME ZONE';
        case 'Boolean':
            return 'BOOLEAN'; // SAPHANA doesn't have built-in boolean
    }
};

/*!
 * Map saphana data types to json types
 * @param {String} saphanaType
 * @param {Number} dataLength
 * @returns {String}
 */
function saphanaDataTypeToJSONType(saphanaType, dataLength) {
    var type = saphanaType.toUpperCase();
    switch (type) {
        case 'BOOLEAN':
            return 'Boolean';
