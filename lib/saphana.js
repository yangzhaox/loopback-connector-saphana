...
exports.initialize = function initializeDataSource(dataSource, callback) {
    ...
    dataSource.client = saphana.createClient(settings);
...
SAPHANA.prototype.create = function (model, data, callback) {
    ...
    this.query(sql, fieldsAndData.params, function (err, results) {
        ...
        sql = 'SELECT ' + this.seqEscaped(model) + '.currval AS "insertId" ' + 'FROM DUMMY';
...
SAPHANA.prototype.updateOrCreate = function (model, data, callback) {
    ...
    Object.keys(data).forEach(function (key) {
        if (props[key] || self.id(model, key)) {
            var k = self.columnEscaped(model, key);
            var v;
            v = self.toDatabase(props[key], data[key], true);
            if (v !== undefined) {
                fieldsNames.push(k);
                fieldValues.push(v);
...
//            callback(err, data);
    ...
    if (this._models[model].properties['id'].generated) {
        sql = 'DROP SEQUENCE ' + self.seqEscaped(model);
        self.query(sql, cb);
    }
...
SAPHANA.prototype.createTable = function (model, cb) {
    ...
    if (this._models[model].properties['id'].generated) {
        sql = 'CREATE SEQUENCE ' + self.seqEscaped(model);
