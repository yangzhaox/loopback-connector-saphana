...
function getTableStatus(model, cb) {
    ...
    this.query('SELECT tc.column_name AS "column", dt.type_name AS "type", tc.is_nullable AS "nullable"' // , data_default AS "Default"'
        + ' FROM "SYS"."TABLE_COLUMNS" AS tc LEFT JOIN "SYS"."DATA_TYPES" AS dt ON tc.data_type_id = dt.type_id'
        + ' WHERE tc.table_name=\'' + this.table(model) + '\'', decoratedCallback);
...
function getColumnsToAdd(model, actualFields) {
    ...
    propNames.forEach(function (propName) {
        ...
        if (!found && propertyHasNotBeenDeleted.call(self, model, propName)) {
            sql.push('ADD (' + addPropertyToActual.call(self, model, propName));
        }
    });
    if (sql.length > 0) {
        sql = [sql.join(', ')];
    ...
    propNames.forEach(function (propName) {
        ...
        if (found && propertyHasNotBeenDeleted.call(self, model, propName)) {
            if (datatypeChanged(propName, found)) {
                sql.push('ALTER (' + modifyDatatypeInActual.call(self, model, propName));
            }
            if (nullabilityChanged(propName, found)) {
                sql.push('ALTER (' + modifyNullabilityInActual.call(self, model, propName));
            }
        }
    });
